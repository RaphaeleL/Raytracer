// g++ -Wall -pedantic -march=native -mfpmath=sse -mavx2 -O3 raytracer.cc
// statistics.cc Ggf. -std=c++11 hinzufügen Für spezifischen FLOAT Wert: -D
// FLOAT=double oder -D FLOAT='long double' hinzufügen Für k-d-Baum Variante:
// g++ -Wall -pedantic -march=native -mfpmath=sse -mavx2  -O3 -D USE_KDTREE
// raytracer.cc statistics.cc kdtree.cc Für optimierten Schnittpunktalgorithmus
// -D OPTIMIZED_INTERSECTS  hinzufügen
#include "kdtree.h"
#include "math.h"
...
static bool reverse_vertice_order =
    false; // switch to counter clockwise orientation of the input file
...
// a value class for material information like the color, ambient and diffuse
// light factors, the amount of reflection and transmission light
class Material {
private:
  Color color;
...
public:
  Material(Color color = Color(1.0, 0.7, 0.8), FLOAT ambient = 0.3,
           FLOAT diffuse = 0.7, FLOAT reflection = 0.0,
...
public:
  Screen(size_t width, size_t height)
      : width(width), height(height),
        buffer(std::unique_ptr<Color[]>(new Color[width * height])) {}
  ...
  Color getPixel(size_t x, size_t y) const { return buffer[x + y * width]; }
};

// a simple camera that can be used for a central projection from an eye to a
// rectangular view port defined by a upper left point and a vector pointing
// downsides and to the right both vectors should be located in the same plane
// and orthogonal to  the eye->center ray (projection is distorted otherwise)
// the viewport has a defined pixel height and width
class Camera {
  Vector<FLOAT, 3> eye, upper_left, down, right;
  FLOAT pixelWidth;
  FLOAT pixelHeight;

public:
  Camera(Vector<FLOAT, 3> eye, Vector<FLOAT, 3> center, Vector<FLOAT, 3> up,
         Vector<FLOAT, 3> right, FLOAT pixelWidth = 1.0,
         FLOAT pixelHeight = 1.0)
      : eye(eye), upper_left(center - right + up), down(-2.0 * up),
        right(2.0 * right), pixelWidth(pixelWidth), pixelHeight(pixelHeight) {
    this->right.normalize();
    this->down.normalize();
  }

  // creates a camera from a given eye->center ray
  // this ray points to a center of a rectangle created by an up and right
  // vector the pixel height and width are calculated from the screen coordiates
  // with the length of the up and right vector
  Camera(Vector<FLOAT, 3> eye, Vector<FLOAT, 3> center, Vector<FLOAT, 3> up,
         Vector<FLOAT, 3> right, Screen &screen)
  ...
  Ray<FLOAT> getRay(size_t x, size_t y) const {
    Vector<FLOAT, 3> direction =
        (upper_left + (x * pixelWidth) * right + (y * pixelHeight) * down) -
        eye;
...
void write_bmp(std::ostream &out, const Screen &screen) {
  unsigned long long size_of_bitmap_data =
      screen.getWidth() * screen.getHeight() * 4;
  out.put(0x42);
  out.put(0x4D); // "BM"
  out.put(0x00);
  out.put(0x00);
  ...
  for (size_t y = 0u; y < screen.getHeight(); y++) {
    for (size_t x = 0u; x < screen.getWidth(); x++) {
      out.put(((unsigned short)(screen.getPixel(x, screen.getHeight() - 1 - y)
                                    .getRed() *
                                255.0)) &
              0xFF);
      out.put(((unsigned short)(screen.getPixel(x, screen.getHeight() - 1 - y)
    ...
    for (size_t padding_bytes = 0u; padding_bytes < screen.getWidth() % 4;
         padding_bytes++) {
      out.put(0x00);
    }
  }
...
public:
  void add(Triangle<FLOAT> triangle) { triangles.push_back(triangle); }
  ...
  // brute force algorithme to determine the nearest (if NEAREST is true)
  // triangle of this scence that has an intersection with the given ray the
  // intersection point p is p = ray.origin + t * ray.direction the u-v
  // parameters are the barycentric coordinates of the intersection within  the
  // triangle

  bool hasNearestTriangle(const Ray<FLOAT> &ray,
                          Triangle<FLOAT> *&nearest_triangle, FLOAT &t,
                          FLOAT &u, FLOAT &v, const bool NEAREST = true) {
    ...
    for (size_t i = 0u; i < triangles.size(); i++) {
      ...
      bool intersect = triangle->intersects(ray.getOrigin(), ray.getDirection(),
                                            t, u, v, minimum_t);
      if (intersect) {
  ...
  // shades the intersection point of the ray with the triangle with the
  // material information the normal vectors and u-v-parameter are used for
  // interpolation
  Color shade(const Ray<FLOAT> &ray, Triangle<FLOAT> &triangle,
              Material &material, FLOAT &t, FLOAT &u, FLOAT &v) {
    Color color = material.getAmbient() * material.getColor();
    FLOAT factor = 1.0 / lights.size();
    Vector<FLOAT, 3> intersection =
        (ray.getOrigin() + 0.9999 * t * ray.getDirection());
    Vector<FLOAT, 3> normal =
        (u * triangle.n1) + (v * triangle.n2) + ((1.0 - u - v) * triangle.n3);
    ...
    for (Light light : lights) {
      ...
      if (!blocked(ray_to_light)) {
        ...
        FLOAT angle =
            std::max<FLOAT>(0.0, light_direction.scalar_product(normal));
        color = color + angle * factor * material.getDiffuse() *
                            material.getColor() * light.getColor();
      }
    }
    return color;
  }
...
void read_wavefront(std::ifstream &in, Scene &scene) {
  ...
  while (in >> c) {
    if (c == 'v' && in.peek() == ' ') {
      ...
      in >> xv;
      if (in.peek() == '/')
        in.ignore(); // no texture coordinates supported
      if (in.peek() == '/') {
        in.ignore();
        in >> xn;
      }
      ...
      if (xn == 0) { // no normals
        scene.add(Triangle<FLOAT>(
            {vertices[xv - 1], vertices[yv - 1], vertices[zv - 1]}));
      } else {
        scene.add(Triangle<FLOAT>({vertices[xv - 1], vertices[yv - 1],
                                   vertices[zv - 1], normals[xn - 1],
                                   normals[yn - 1], normals[zn - 1]}));
      }
      no_of_triangles++;
  ...
  std::cerr << "memory used for all triangles [byte] : "
            << no_of_triangles * sizeof(Triangle<FLOAT>) << std::endl;
}
...
#ifndef USE_KDTREE
      bool hasNearestTriangle =
          scene.hasNearestTriangle(ray, nearest_triangle, t, u, v);
#else
      bool hasNearestTriangle = tree->hasNearestTriangle(
          ray.getOrigin(), ray.getDirection(), nearest_triangle, t, u, v);
#endif
...
void parse_arguments(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "ro:", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 'f':
      ...
      break;
    case 'r':
      reverse_vertice_order = true;
      break;
    case 'w':
      resolution_x = std::stoi(optarg);
      break;
    case 'h':
      resolution_y = std::stoi(optarg);
      break;
    case 'o':
      output_bmp_file_name = std::string(optarg);
      break;
    case 'n':
      write_ppm_to_stdout = false;
      break;
    case 'u':
      std::cout << "Aufruf: " << argv[0] << " [options]" << std::endl;
      std::cout << "  --file FILE          wavefront obj input file "
                   "('example/teapot.obj')"
                << std::endl;
      std::cout << "  --width WIDTH        horizontal resolution (256)"
                << std::endl;
      std::cout << "  --height HEIGHT      vertical resolution (256)"
                << std::endl;
      std::cout << "  --help               this help text" << std::endl;
      std::cout << "  --no_ppm             no PPM output is generated"
                << std::endl;
      std::cout << "  -o BMP               BMP output file ('output.bmp')"
                << std::endl;
      std::cout << "  -r                   reverse vertice order of triangles "
                   "in input file"
                << std::endl;
      exit(0);
    }
...
int main(int argc, char *argv[]) {
  ...
  std::ofstream output(output_bmp_file_name,
                       std::ofstream::binary); // for windows
  ...
  Camera camera(Vector<FLOAT, 3>({0.05, 1.0, 200.0}),
                ...
                Vector<FLOAT, 3>({1.6, 0.0, 0.0}), screen);
  ...
  std::unique_ptr<KDTree> tree =
      std::unique_ptr<KDTree>(KDTree::buildTree(triangles));
