...
public:
  ...
  // the point to the intersecting triangle nearest to the eye is written to
  // nearest_triangle the value t for the intersection point is set the
  // barycentric coordinates of the intersection are set to u and v minimum_t is
  // the parametic value for the current nearst intersection
  bool hasNearestTriangle(Vector<FLOAT, 3> eye, Vector<FLOAT, 3> direction,
                          Triangle<FLOAT> *&nearest_triangle, FLOAT &t,
                          FLOAT &u, FLOAT &v, FLOAT minimum_t = INFINITY);
};
